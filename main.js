...
mainHeader.classList.add("subtitle");
...
let paragraph = overviewSection.querySelector("p");

// 2.3: Get the class name (or names) of the element you found in the previous step.
//    Hint: DOM elements have a property that represents their classes.
let existingClasses = paragraph.classList;
console.log( "Class Names:", existingClasses);
...
let featuresList = featuresSection.querySelector("ul");
...
let featuresList_alt = document.getElementById("features").querySelector("ul");
...
let children =featuresList_alt.children.length;
...
while (featuresList_alt.children.length > 0) {
    featuresList_alt.removeChild(featuresList_alt.lastChild);
}